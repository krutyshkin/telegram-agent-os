---
name: hermes-bulk-skill-install
description: Bulk install Hermes skills from a role-based manifest with backups, custom
  fallbacks, verification, and registry generation.
version: 1.0.0
metadata:
  telegram_agent_os:
    public_template: true
    source_skill: hermes-bulk-skill-install
---

# Hermes Bulk Skill Install

## Purpose

This public skill template documents the `hermes-bulk-skill-install` capability for a Telegram Agent OS deployment.

## When to use

Use this skill when the active role needs the procedures, checks, or domain knowledge associated with `hermes-bulk-skill-install`.
