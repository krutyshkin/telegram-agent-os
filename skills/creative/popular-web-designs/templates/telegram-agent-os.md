# Design System: Emerald Terminal
...
- **Emerald Mint** (`#2fd6a1`): The button-text variant of the brand green — slightly warmer and more readable than pure Signal Green, used specifically for CTA text on dark surfaces.
- **Tailwind Emerald** (`#10b981`): The ecosystem-standard green used at low opacity (30%) for subtle background tints and link defaults. Bridges Emerald Terminal's custom palette with Tailwind's utility classes.
...
### Gradient System
- **Green Signal Glow**: `drop-shadow(0 0 2px #00d992)` animating to `drop-shadow(0 0 8px #00d992)` — creates a pulsing "electric charge" effect on the Emerald Terminal bolt logo and interactive elements. The glow expands and contracts like a heartbeat.
...
- Text: Emerald Mint (`#2fd6a1`)
- Padding: comfortable (12px 16px)
- Border: none visible (outline-based focus indicator)
- Outline: Emerald Mint (`rgb(47, 214, 161)`)
...
- The npm install command (`npm create agent-os-app@latest`) is presented as a code block rather than an input field
- Inferred style: Carbon Surface background, Warm Charcoal border, Emerald Mint focus ring, Snow White text

### Navigation
- Sticky top nav bar on Abyss Black canvas
- Logo: Emerald Terminal bolt icon with animated green glow (`drop-shadow` cycling 2px–8px)
...
- A prominent code snippet (`npm create agent-os-app@latest`) styled as a copyable command
...
- Connection lines use Emerald Terminal green variants
...
- **Border-defined separation**: Rather than relying solely on whitespace, Emerald Terminal uses the Warm Charcoal border system (`#3d3a39`) to delineate content zones. The border IS the whitespace signal.
...
**Shadow Philosophy**: Emerald Terminal communicates depth primarily through **border weight and color**, not shadows. The standard `1px solid #3d3a39` border IS the elevation — adding a `3px` border weight or switching to green (`#00d992`) communicates importance more than adding shadow does. When shadows do appear, they're either warm and diffused (Level 4) or cinematic and dramatic (Level 5) — never medium or generic.

### Decorative Depth
- **Green Signal Glow**: The Emerald Terminal bolt logo pulses with a `drop-shadow` animation cycling between 2px and 8px blur radius in Emerald Signal Green. This is the most distinctive decorative element — it makes the logo feel "powered on."
...
- Use Emerald Mint (`#2fd6a1`) for button text on dark surfaces — it's more readable than pure Signal Green
...
- Button Text: "Emerald Mint (#2fd6a1)"
...
- "Design a navigation bar on Abyss Black (#050507) with the bolt-style product mark (bolt icon with animated green glow) on the left, nav links in Inter at 14px weight 500 in Snow White, and a green CTA button (Carbon Surface bg, Emerald Mint text) on the right. Add a 1px solid Warm Charcoal bottom border."
