---
name: hermes-skill-namespace-migration
description: Rename or de-namespace Hermes skills in bulk, merge duplicate original
  skill names, update AGENTS.md references, back up safely, and verify the active
  skill registry.
version: 1.0.0
metadata:
  telegram_agent_os:
    public_template: true
    source_skill: hermes-skill-namespace-migration
---

# Hermes Skill Namespace Migration

## Purpose

This public skill template documents the `hermes-<SECRET>` capability for a Telegram Agent OS deployment.

## When to use

Use this skill when the active role needs the procedures, checks, or domain knowledge associated with `hermes-<SECRET>`.
